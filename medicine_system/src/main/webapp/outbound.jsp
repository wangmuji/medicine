...
<html>
    <head>
        	...
        	if(judge1&&judge2)
        		{
	        		...
	        		window.location.href="OutboundServlet?Time1="+time1+"&Time2="+time2;
        ...
        function DetailedInfor_S(s_id,id,s_time,name,s_num,s_total,s_reason,price,account) 
        {
            window.location.href="DetailedInforSServlet?S_ID="+s_id+"&ID="+id+"&S_time="+s_time+"&Name="+name+"&S_Num="+s_num+"&S_Total="+s_total+"&S_Reason="+s_reason+"&Price="+price+"&Account="+account;
        }
        </script>
    </head>
    <body>
        ...
        <table>
            <tbody>
                ...
                <c:forEach var="S_message" items="${S_Messages}">
                    <tr>                    
                        <td>${S_message.s_id}</td>
                        <td>${S_message.name }</td>
                        <td>${S_message.s_num }</td>
                        <td>${S_message.s_time }</td>
                        <td><button onclick="DetailedInfor_S('${S_message.s_id}','${S_message.id}','${S_message.s_time}','${S_message.name}','${S_message.s_num}','${S_message.s_total}','${S_message.s_reason}','${S_message.price}','${S_message.account}')" herf="javaScript:void(0)">查看</button></td>  
                    </tr>
                </c:forEach>
        ...
        <div id="navigate">
                ...
                <div id="tpbt">
                	...
                	<c:if test="${page.pageIndex>1  }">
                		<a href="${pageContext.request.contextPath}/outboundServlet?pageIndex=${page.pageIndex-1}"><button class="gre" id="lastpage">上一页</button></a>
                    ...
                    <c:if test="${page.pageIndex < page.totalPages }">
                    	<a href="${pageContext.request.contextPath}/outboundServlet?pageIndex=${page.pageIndex+1}"><button class="red" id="nextpage">下一页</button></a>
            		</c:if>
            </div>
        </div>
